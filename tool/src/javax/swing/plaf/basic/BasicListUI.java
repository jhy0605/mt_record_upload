...
public class BasicListUI extends ListUI
{
    ...
    private class Handler implements FocusListener, KeyListener,
        ...
        public void focusGained(FocusEvent e) {
        /* The below code is added to make the focus visible to the users when  //IBM-awt_oracle_giveback
           no item is selected in the list. So here we will verify whether the lead //IBM-awt_oracle_giveback
           selection index to be -1 to make the lead selection index to 0 so that the //IBM-awt_oracle_giveback
           focus will be shown visibly. Note: We will clear the selection here since our //IBM-awt_oracle_giveback
           motive is not to select the item of the list but to show visual focus //IBM-awt_oracle_giveback
        */                                                                      //IBM-awt_oracle_giveback
        ListSelectionModel lsm=list.getSelectionModel();                        //IBM-awt_oracle_giveback
        if((list.getModel().getSize()>0) &&                                     //IBM-awt_oracle_giveback
           ((lsm.getLeadSelectionIndex()<0) ||                                  //IBM-awt_oracle_giveback
                (lsm.getLeadSelectionIndex()>=list.getModel().getSize()))){     //IBM-awt_oracle_giveback
            lsm.setAnchorSelectionIndex(0);                                     //IBM-awt_oracle_giveback
            lsm.setLeadSelectionIndex(0);                                       //IBM-awt_oracle_giveback
            lsm.clearSelection();                                               //IBM-awt_oracle_giveback
        }                                                                       //IBM-awt_oracle_giveback
            repaintCellFocus();
        }
