...
class Bits {                            // package-private
    ...
    private static boolean allowShrinkage = false;
    ...
    static void reserveMemory(long size, int cap) {
        synchronized (Bits.class) {
            if (!memoryLimitSet && VM.isBooted()) {
                ...
                /* PR 126846::	
                 * By default the dbb memory will not shrink once it is expanded. 
                 * However, for specific workloads, the memory growth can be high as we proactively expand the memory to reduce frequent gc. 
                 * If application wants to enable shrink when memory consumption is low, use ibm.dbb.allowShrinkage. 
                 * This is an undocumented internal property which can be used as a mitigation in case of any unexpected memory growth due to dbb.
                 */
                allowShrinkage = java.security.AccessController.doPrivileged(
                                    new GetBooleanAction("ibm.dbb.allowShrinkage")).booleanValue();
                /*JSE-2677::
            ...
            if (cap <= maxMemory - totalCapacity) {
                ...
                count++;
                //shrinkage
                if(allowShrinkage && !xxOptionUsed && totalCapacity <= 0.2 * maxMemory) {               
                    if(maxMemory >= MB96)
